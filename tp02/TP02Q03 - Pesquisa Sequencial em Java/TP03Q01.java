...
class List {
    ...
    public boolean searchName(String name) {
        boolean answ = false;
        for (Node i = first.prox; i != null; i = i.prox, List.comparisons++) {
...
public class TP03Q01 {
    private static void writeMatricula(long totalTime, int comparisons) {
        String OUTPUT_FILE_NAME = "matrícula_sequencial.txt";
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(OUTPUT_FILE_NAME))) {
            writer.write("Matrícula: 746733\tNumero de comparacoes: " + comparisons
    ...
    public static void main(String [] args) throws Exception {    
        ...
        while(!(aux.contains("FIM"))) {
            ...
            if(playerList.searchName(aux.trim())) {
        ...
        writeMatricula(totalTime, List.comparisons);
        sc.close();
    }
}