...
class List {
    ...
    public void sortByBirthYearAndName() {
        ...
        while (current != null) {
            ...
            if (sortedList == null || 
                current.player.getBirthYear() < sortedList.player.getBirthYear() || 
                (current.player.getBirthYear() == sortedList.player.getBirthYear() && 
                current.player.getName().compareTo(sortedList.player.getName()) <= 0)) {
                
                current.next = sortedList;
                sortedList = current;
            } else {
                Node prevSorted = sortedList;
                while (prevSorted.next != null && 
                    (current.player.getBirthYear() > prevSorted.next.player.getBirthYear() || 
                    (current.player.getBirthYear() == prevSorted.next.player.getBirthYear() && 
                    current.player.getName().compareTo(prevSorted.next.player.getName()) > 0))) 
...
public class TP02Q07 {
    ...
    public static void main(String [] args) throws Exception {
        ...
        playerList.sortByBirthYearAndName();
